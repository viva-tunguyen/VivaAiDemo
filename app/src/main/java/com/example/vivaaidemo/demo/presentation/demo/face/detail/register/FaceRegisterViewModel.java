...
public class FaceRegisterViewModel extends BaseViewModel {
    ...
    public void face(Context context, String link, String name, String email, String employeeCode, boolean isFaceMask) {
        if (TextUtils.isEmpty(link)) {
            message.setValue("Please fill some text");
            return;
        }

        boolean isGrant = checkPermission(context);
        if (!isGrant) {
//            Log.w("")
            return;
        }
        ...
        manager.runBackground(() -> manager.face().faceRegister(callback, link, name, email, employeeCode, isFaceMask));
    }
